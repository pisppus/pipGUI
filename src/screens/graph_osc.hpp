...
{
  ui.clear(0x0000);
  ui.configGraphScope().screen(graphOsc).rate(320).timebase(700);
  ...
  ui.drawGraphGrid().pos(center, center).size(200, 170).radius(13).direction(Oscilloscope).bgColor(ui.rgb(8, 8, 8)).speed(1.0f);
  ui.setGraphScale(false);

  const uint16_t sr = ui.graphRate(graphOsc);
  const uint16_t tb = ui.graphTimebase(graphOsc);
  const uint16_t vs = ui.graphSamplesVisible(graphOsc);
  ui.drawText().text(String(sr) + "Hz  " + String(tb) + "ms  " + String(vs) + " samples")
  ...
  const uint16_t sampleCount = (vs > kMaxOscSamples) ? kMaxOscSamples : vs;
  ...
  const float dt = (sr > 0) ? (1.0f / (float)sr) : 0.01f;
