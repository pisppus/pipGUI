...
from menu import choose, choose_existing_path, prompt_multiline, prompt_text
from util import (
    ensure_pynacl,
...
def main() -> int:
    ...
    ap.add_argument("--sign-key", default=None, help="Path to Ed25519 seed (hex) created by key.py")
    ...
    if interactive:
        ...
        if len(compact) not in (4, 5):
            raise SystemExit(
                ...
                "  python tools/ota/release.py --.pio/build/<env>/firmware.bin --stable --\"Title\" --1.2.3 --https://<domain>/fw [--\"Notes\"]\n"
